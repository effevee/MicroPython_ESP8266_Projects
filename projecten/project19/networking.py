    ...
    def connectWIFI():
        ...
        if not net.config.wlan.isconnected():
            ...
            while not net.config.wlan.isconnected():
                pass
        # if static_ip is defined in netconfig then
        # change ip adress obtained by DHCP with the static adress
    ...
    def start():
        try:
            ...
            net.config.sSocket = socket.socket(socket.AF_INET,
                                               socket.SOCK_STREAM)
            net.config.sSocket.setsockopt(socket.SOL_SOCKET,
